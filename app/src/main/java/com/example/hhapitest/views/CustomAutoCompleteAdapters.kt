...
class CustomAutoCompleteAdapterAreas(
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        if (convertView == null) {
            ...
            val newConvertView =
                layoutInflater.inflate(R.layout.auto_complete_text_view, parent, false)
...
class CustomAutoCompleteAdapterEmployers(
    private val context: Context,
    val viewModel: CreateRequestViewModel
) : BaseAdapter(), Filterable {
                    ...
                    if (constraint != null && constraint.isNotEmpty()) {
                        ...
                        currentTask?.enqueue(MainThreadDispatcher()) { result ->
                            listItem = if (result is SuccessResult && result.data != "") {
                                try {
                                    JsonImpl.getListEmployersForRequestList(result.data) ?: listOf()
                                } catch (e: Exception) {
                                    listOf()
                                }
                            } else listOf()
